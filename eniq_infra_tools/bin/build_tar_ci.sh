...
if [ "$Module" == "storage" ]
then
        perl eniq_infra_tools/bin/build_rpm_ci -r $Rstate -m ERICstmapi		
	perl eniq_infra_tools/bin/build_rpm_ci -r $Rstate -m ERICstmnas
        perl eniq_infra_tools/bin/build_rpm_ci -r $Rstate -m ERICstorapi
#	perl eniq_infra_tools/bin/build_rpm -r $Rstate -m ERICmigration
	        mkdir $WORKSPACE"/rpm_files"
		cp -r eniq_infra_delivery/ERICstmapi/RPMS/ERICstmapi-$Rstate.rpm $WORKSPACE"/rpm_files/"
		cp -r eniq_infra_delivery/ERICstmnas/RPMS/ERICstmnas-$Rstate.rpm $WORKSPACE"/rpm_files/"
		cp -r eniq_infra_delivery/ERICstorapi/RPMS/ERICstorapi-$Rstate.rpm $WORKSPACE"/rpm_files/"
...
#		cp -r ../../eniq_infra_delivery/ERICmigration/RPMS/ERICmigration-$Rstate.rpm .
		...
		mkdir -p $WORKSPACE"/tar_files"
		cp -r storage_$Rstate.tar.gz $WORKSPACE"/tar_files"
				
elif [ "$Module" == "omtools" ]
then
		perl eniq_infra_tools/bin/build_rpm_ci -r $Rstate -m ERICkickstart
		mkdir $WORKSPACE"/rpm_files"
		cp -r eniq_infra_delivery/ERICkickstart/RPMS/ERICkickstart-$Rstate.rpm $WORKSPACE"/rpm_files/"
		cd om_linux/omtools
		sed -i "s/Rev/$Rstate/g" cxp_info
		cd eric_kickstart
		rm -rf *.rpm
		...
		mkdir -p $WORKSPACE"/tar_files"
		cp -r omtools_$Rstate.tar.gz $WORKSPACE"/tar_files"
		
elif [ "$Module" == "security" ]	
then
		perl eniq_infra_tools/bin/build_rpm_ci -r $Rstate -m ERICnodehardening
		mkdir $WORKSPACE"/rpm_files"
		cp -r eniq_infra_delivery/ERICnodehardening/RPMS/ERICnodehardening-$Rstate.rpm $WORKSPACE"/rpm_files/"
		...
		mkdir -p $WORKSPACE"/tar_files"
		cp -r security_$Rstate.tar.gz $WORKSPACE"/tar_files"
...
then
		...
		mkdir -p $WORKSPACE"/tar_files"
		cp -r bootstrap_$Rstate.tar.gz $WORKSPACE"/tar_files"
...
then
		...
		createrepo $WORKSPACE"/om_linux/EMC/"
		tar -cvf EMC_$Rstate.tar ../EMC
		gzip EMC_$Rstate.tar
		mkdir -p $WORKSPACE"/tar_files"
		cp -r EMC_$Rstate.tar.gz $WORKSPACE"/tar_files"
...
then
		...
		mkdir -p $WORKSPACE"/tar_files"
		cp -r HWcomm_$Rstate.tar.gz $WORKSPACE"/tar_files"
...
then
		...
		mkdir -p $WORKSPACE"/tar_files"
		cp -r patch_$Rstate.tar.gz $WORKSPACE"/tar_files"
...
then
                ...
                mkdir -p $WORKSPACE"/tar_files"
                cp -r blade_$Rstate.tar.gz $WORKSPACE"/tar_files"

elif [ "$Module" == "sanconfig" ]
then
                perl eniq_infra_tools/bin/build_rpm_ci -r $Rstate -m ERICstorconfig
		mkdir $WORKSPACE"/rpm_files"
		cp -r eniq_infra_delivery/ERICstorconfig/RPMS/ERICstorconfig-$Rstate.rpm $WORKSPACE"/rpm_files/"
		...
		mkdir -p $WORKSPACE"/tar_files"
		cp -r sanconfig_$Rstate.tar.gz $WORKSPACE"/tar_files"


elif [ "$Module" == "mwshealthcheck" ]
then
                perl eniq_infra_tools/bin/build_rpm_ci -r $Rstate -m ERICmwshealthcheck
                mkdir $WORKSPACE"/rpm_files"
                cp -r eniq_infra_delivery/ERICmwshealthcheck/RPMS/ERICmwshealthcheck-$Rstate.rpm $WORKSPACE"/rpm_files/"
                ...
                mkdir -p $WORKSPACE"/tar_files"
                cp -r mwshealthcheck_$Rstate.tar.gz $WORKSPACE"/tar_files"
...
then
                ...
                mkdir -p $WORKSPACE"/tar_files"
                cp -r vaupgrade_$Rstate.tar.gz $WORKSPACE"/tar_files"
